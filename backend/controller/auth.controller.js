...
export const verifyOtp = async (req, res) => {
  try {
    ...
    ((user.isOtpVerified = true), (user.resetOtp = undefined));
    user.otpExpires = undefined;
    await user.save();
    return res.status(200).json({ messgae: "Otp verify successfully" });
  } catch (error) {
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!user || !user.isOtpVerified) {
      return res.status(400).json({ messgae: "Otp verification requried" });
    }
